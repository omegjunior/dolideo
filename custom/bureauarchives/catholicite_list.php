...
/**
 *   	\file       catholicite_list.php
 *		\ingroup    bureauarchives
 *		\brief      List page for catholicite
 */
...
require_once __DIR__.'/class/catholicite.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('bureauarchives', 'catholicite', 'write');
	$permissiontodelete = $user->hasRight('bureauarchives', 'catholicite', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("bureauarchives")) {
	accessforbidden('Module bureauarchives not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Catholicite';
	$objectlabel = 'Catholicite';
	$uploaddir = $conf->bureauarchives->dir_output;
...
$title = $langs->trans("Catholicites");
//$help_url = "EN:Module_Catholicite|FR:Module_Catholicite_FR|ES:Módulo_Catholicite";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/bureauarchives/catholicite_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/bureauarchives/catholicite_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendCatholiciteRef";
$modelmail = "catholicite";
$objecttmp = new Catholicite($db);
