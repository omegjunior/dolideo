...
/**
 * \file    test/phpunit/BureauArchivesFunctionalTest.php
...
/**
 * Class BureauArchivesFunctionalTest
 *
 ...
 *
 * @package Testbureauarchives
 */
class BureauArchivesFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modBureauArchives')->click();
		} else {
			// Disable the module
			$this->byHref('modBureauArchives')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/bureauarchives/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('bureauarchives/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/bureauarchives/admin/about.php');
		$this->authenticate();
		return $this->assertContains('bureauarchives/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('bureauarchiveswidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modBureauArchives_BureauArchivesTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modBureauArchives_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
