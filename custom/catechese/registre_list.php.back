...
/**
 *   	\file       registre_list.php
 *		\ingroup    catechese
 *		\brief      List page for registre
 */
...
require_once __DIR__.'/class/registre.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('catechese', 'registre', 'write');
	$permissiontodelete = $user->hasRight('catechese', 'registre', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("catechese")) {
	accessforbidden('Module catechese not enabled');
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'Registre';
	$objectlabel = 'Registre';
	$uploaddir = $conf->catechese->dir_output;
...
$title = $langs->trans("Registres");
//$help_url = "EN:Module_Registre|FR:Module_Registre_FR|ES:Módulo_Registre";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/catechese/registre_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/catechese/registre_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendRegistreRef";
$modelmail = "registre";
$objecttmp = new Registre($db);
