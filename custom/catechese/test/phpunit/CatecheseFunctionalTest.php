...
/**
 * \file    test/phpunit/CatecheseFunctionalTest.php
 * \ingroup catechese
 ...
 *
 * @package Testcatechese
 */
class CatecheseFunctionalTest extends \PHPUnit_Extensions_Selenium2TestCase
{
	...
	public function testModuleEnabled()
	{
		...
		if (strstr($module_status_image->attribute('src'), 'switch_off.png')) {
			// Enable the module
			$this->byHref('modCatechese')->click();
		} else {
			// Disable the module
			$this->byHref('modCatechese')->click();
			// Reenable the module
	...
	{
		$this->url('/custom/catechese/admin/setup.php');
		$this->authenticate();
		return $this->assertContains('catechese/admin/setup.php', $this->url(), 'Configuration page');
	}
	...
	{
		$this->url('/custom/catechese/admin/about.php');
		$this->authenticate();
		return $this->assertContains('catechese/admin/about.php', $this->url(), 'About page');
	}
	...
	public function testBoxDeclared()
	{
		...
		$this->authenticate();
		return $this->assertContains('catechesewidget1', $this->source(), "Box enabled");
	}
	...
	public function testTriggerDeclared()
	{
		...
		return $this->assertContains(
			'interface_99_modCatechese_CatecheseTriggers.class.php',
	...
	public function testTriggerEnabled()
	{
		...
		return $this->assertContains(
			'tick.png',
			$this->byXPath('//td[text()="interface_99_modCatechese_MyTrigger.class.php"]/following::img')->attribute('src'),
			"Trigger enabled"
		);
	}
