...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('dolideo', 'demandecelebration', 'write');
	$permissiontodelete = $user->hasRight('dolideo', 'demandecelebration', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("dolideo")) {
	accessforbidden('Module dolideo not enabled');
}
...
if (empty($reshook)) {
	...
	$uploaddir = $conf->dolideo->dir_output;
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/dolideo/demandecelebration_card.php', 1).'?id='.((int) $id));
	exit;
}
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/dolideo/demandecelebration_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);
