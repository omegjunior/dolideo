...
/**
 *   	\file       demandecelebration_list.php
 *		\ingroup    secretariatmesse
 *		\brief      List page for demandecelebration
 */
...
require_once __DIR__.'/class/demandecelebration.class.php';
...
if ($enablepermissioncheck) {
	...
	$permissiontoadd = $user->hasRight('secretariatmesse', 'demandecelebration', 'write');
	$permissiontodelete = $user->hasRight('secretariatmesse', 'demandecelebration', 'delete');
} else {
	$permissiontoread = 1;
	$permissiontoadd = 1;
...
if (!isModEnabled("secretariatmesse")) {
	accessforbidden('Module secretariatmesse not enabled');
}
if (!$permissiontoread) {
	accessforbidden();
}
...
if (empty($reshook)) {
	...
	// Mass actions
	$objectclass = 'DemandeCelebration';
	$objectlabel = 'DemandeCelebration';
	$uploaddir = $conf->secretariatmesse->dir_output;
...
$title = $langs->trans("DemandeCelebrations");
//$help_url = "EN:Module_DemandeCelebration|FR:Module_DemandeCelebration_FR|ES:Módulo_DemandeCelebration";
$help_url = '';
$morejs = array();
$morecss = array();
...
if ($num == 1 && getDolGlobalInt('MAIN_SEARCH_DIRECT_OPEN_IF_ONLY_ONE') && $search_all && !$page) {
	...
	$id = $obj->rowid;
	header("Location: ".dol_buildpath('/secretariatmesse/demandecelebration_card.php', 1).'?id='.((int) $id));
	exit;
}
...
llxHeader('', $title, $help_url, '', 0, 0, $morejs, $morecss, '', 'mod-secretariatmesse page-list bodyforlist');	// Can use also classforhorizontalscrolloftabs instead of bodyforlist for no horizontal scroll
...
$newcardbutton .= dolGetButtonTitle($langs->trans('New'), '', 'fa fa-plus-circle', dol_buildpath('/secretariatmesse/demandecelebration_card.php', 1).'?action=create&backtopage='.urlencode($_SERVER['PHP_SELF']), '', $permissiontoadd);

print_barre_liste($title, $page, $_SERVER["PHP_SELF"], $param, $sortfield, $sortorder, $massactionbutton, $num, $nbtotalofrecords, 'object_'.$object->picto, 0, $newcardbutton, '', $limit, 0, 0, 1);

// Add code for pre mass action (confirmation or email presend form)
$topicmail = "SendDemandeCelebrationRef";
$modelmail = "demandecelebration";
$objecttmp = new DemandeCelebration($db);
